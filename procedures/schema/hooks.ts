/**
 * Vendored from Archon `packages/workflows/src/schemas/hooks.ts`.
 * See ARCHON_VERSION.md for sync sha.
 *
 * Zod schemas for per-node hook configuration.
 *
 * NOTE: Hook execution is not implemented in pi-chamber's procedures runtime
 * (Phase 1 — see compatibility table in procedures/README). The schema is
 * vendored so that workflows authored for Archon parse cleanly here and so
 * the loader can warn about ignored hook fields.
 */
import { z } from 'zod';
